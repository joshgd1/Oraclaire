...
Date: 2026-05-14
Status: CODIFY COMPLETE — Sprint 1 closed
...
**Cost of ignoring it:** Without the RE-DO, threshold 0.40 would have been selected to pass the FP floor — missing 5 employees with Burn Rate ≥ 0.60 at a FN exposure of $20,000-$105,000 per year (D3 cost model: 5 × $4,000-$21,000). The model fix (interaction terms) reduced FP from 20.1% to a level where 0.35 was sufficient, keeping those 5 employees in the scored population. The threshold was still moved (0.30 → 0.35), but the move was smaller and better-grounded because the model was fixed first.

**Near-miss prevented:** The band review condition in D25 ("confirm no BR ≥ 0.60 employees in 0.30-0.40 band") prevented a premature threshold move to 0.40 that would have excluded the highest-risk employees from the product — a threshold adjustment that would have made the numbers look clean while silently dropping the very population the product exists to serve.
...
**Cost of ignoring it:** The participation decay model (D5) showed that above 20% FP, participation drops 40-60% by cycle 3. At that point, the buyer's ROI collapses because there are insufficient assessments to produce reliable team-level aggregates. The product's architecture was built around this constraint: employee-first data visibility (D12), 24-hour gate before HR sees scores (D13), revocable consent (D1), and SHAP-matched curated content (D12). Ignoring the misalignment would mean shipping a product that the HR Director loves in the demo but that employees abandon after one cycle.

**Near-miss prevented:** The two-tier scoring architecture (D1) was adopted specifically because a single-tier model that gave HR direct access to individual scores would have produced immediate trust erosion. Without it, the product would have shipped as "employer surveillance of employee mental health" — accurate as a description, fatal as a product.
...
## What Would Have Shipped Without the Playbook

- **Without Phase 2 data audit:** A fake employee (fakeEMP_007) would have been included in the training set — producing a model trained on synthetic data that does not represent any real employee. The data audit (`0018-DECISION-d19-pipeline-integrity-clearance.md`) caught and removed the contamination before it reached any model.

- **Without Phase 4 MFS gate (D16):** A model where mental_fatigue_score drives 97.4% of predictions (XGBoost) or 46.9% (RF on full dataset) would have shipped — producing SHAP waterfalls that tell every employee "your fatigue score determines your risk" with no actionable nuance. The 40% gate disqualified XGBoost and triggered the RF RE-DO.

- **Without Phase 7 full-dataset RE-DO:** The interaction features `tenure_fatigue` and `tenure_workload` would not exist. MFS SHAP would be 46.9% — above the 40% gate. The SHAP waterfall hard gate would fire, blocking employee-facing explanations. The product would be deployable to staging but unable to surface its primary user-facing feature.

- **Without Phase 6 threshold band review condition:** Threshold would have moved to 0.40 to pass the FP ceiling — silently excluding 5 employees with Burn Rate ≥ 0.60 (including one at BR=0.84) at a FN cost exposure of $20,000-$105,000/year. The band review condition in D25 caught this before the threshold was locked.

---

## Sprint 2 Opens With

1. **Gender fairness audit on full Kaggle dataset (~22,750 rows)** — pre-registered commitment from D11 Nuance 4 and D17 Phase 5 pick. Phase 7 Sweep 3 confirmed 0% Gender proxy effect on the 12-row sample, but the sample is too small for meaningful fairness metrics. This is not optional — it is a pre-registered commitment (D26, phase_8_gate.md Step 2).

2. **Per-tier Brier validation on full dataset** — Phase 7 Sweep 4 flagged Moderate and High tier Brier failures (n=1 each on 12 rows). Full-dataset per-tier Brier against the 0.15 floor is a Sprint 2 validation gate. If breached: Platt scaling applied to the failing tier (D26 disposition S4-F2).

3. **Threshold B senior validation on HRIS data** — THRESHOLD_B remains at 0.30 provisional because senior-tier FN/FP rates have never been evaluated on real workforce data. The pilot customer's HRIS data must validate the 10% FN target and 20% FP ceiling on senior employees before Threshold B is confirmed (D17, D26).

---

Sprint 1 codify complete. 5 lessons extracted. 4 near-misses documented. 3 Sprint 2 open items named.
